...
from src.schemas.post import FeedResponse
from src.services import user as user_service, post as post_service
...
async def search(q: str, limit: int = Query(20, le=50), current_user: User = Depends(get_current_user)):
...
@router.get("/{user_id}/posts", response_model=FeedResponse)
async def get_user_posts(
    user_id: str,
    cursor: Optional[str] = Query(None),
    limit: int = Query(20, le=50),
):
    """Get all posts by a user (profile grid)."""
    return await post_service.get_user_posts(user_id, cursor, limit)
...
async def get_followers(
    ...
    users = await user_service.get_followers(user_id, limit, skip)
    return [u.to_public() for u in users]
